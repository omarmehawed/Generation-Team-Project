...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div x-data="{ tab: 'members' }" class="space-y-6">
            <div class="flex flex-wrap gap-2 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-2 shadow-sm sticky top-0 z-20">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($members as $m)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-400">
                                                {{ $m->technical_role ?? 'Member' }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <button onclick="openEvaluationModal('{{ $m->id }}', '{{ $m->user->name }}', 'member')" class="rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-2 text-xs font-black hover:bg-gray-50 transition-all text-gray-700 dark:text-gray-300">
                                                Evaluate
                                            </button>
                                        </td>
                                    </tr>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($workshops as $w)
                        <div class="rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm">
                            ...
                            <h3 class="text-lg font-black text-gray-900 dark:text-gray-100 mb-4">{{ $w->name }}</h3>
                            <a href="{{ route('final_project.dashboard', $team->id) }}#workshops" class="block w-full rounded-xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 py-3 text-[10px] font-black text-gray-700 dark:text-gray-300 hover:bg-indigo-50 hover:text-indigo-600 transition-colors text-center uppercase tracking-widest">
                                View Attendance
                            </a>
                        </div>
                    @empty
