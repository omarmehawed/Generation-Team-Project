...
<div class="min-h-screen bg-gray-50 text-gray-900 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <select onchange="window.location.href='?period_id='+this.value" 
                        class="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-black focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm transition-all">
                ...
                <a href="{{ route('evaluation.export', ['team' => $team->id, 'period_id' => request('period_id')]) }}" class="rounded-xl bg-gray-100 text-gray-700 px-4 py-2 text-sm font-medium shadow hover:bg-gray-200 transition">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
            ...
            <div class="rounded-2xl bg-white border border-gray-200 p-5 shadow-sm">
                <p class="text-sm text-gray-500">Sub Leaders</p>
                ...
                <div class="rounded-2xl bg-white border border-blue-200 p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-3 gap-3 mt-4">
                        <div class="rounded-xl bg-gray-50 p-3">
                            <p class="text-xs text-gray-500">Sub Leaders</p>
        ...
        @if($showSoftwareOnly)
            <div class="rounded-2xl bg-white border border-blue-200 p-5 shadow-sm mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-blue-600 font-medium">Software Team</p>
                        <h3 class="text-xl font-bold mt-1">{{ $softwareStats['members_count'] ?? 0 }} Members</h3>
        ...
        @if($showHardwareOnly)
            <div class="rounded-2xl bg-white border border-amber-200 p-5 shadow-sm mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-amber-600 font-medium">Hardware Team</p>
                        <h3 class="text-xl font-bold mt-1">{{ $hardwareStats['members_count'] ?? 0 }} Members</h3>
        ...
        <div x-data="{ tab: 'members' }" class="space-y-6">
            <div class="flex flex-wrap gap-2 rounded-2xl bg-white border border-gray-200 p-2 shadow-sm">
            ...
            <div x-show="tab==='members'" x-transition>
                <div class="rounded-2xl bg-white border border-gray-200 shadow-sm">
                    <div class="p-4 border-b border-gray-200 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                        ...
                        <form action="{{ request()->url() }}" method="GET" class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                            ...
                            @if($isLeader || str_contains($viewRole, 'vice_leader'))
                                <select name="team_filter" onchange="this.form.submit()" 
                                    class="rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs font-bold text-indigo-600 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            ...
                            <div class="relative w-full lg:w-64">
                                <input type="text" name="search" value="{{ $currentSearch ?? '' }}" placeholder="Name, Email, or Academic #" 
                                    class="w-full rounded-xl border border-gray-300 bg-white pl-4 pr-10 py-2 text-xs font-medium focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm text-left">
                            ...
                            <tbody>
                                @forelse(($members ?? collect()) as $member)
                                    <tr class="border-t border-gray-200 group hover:bg-gray-50">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    @if($isLeader || $isGeneralVice || $isSoftwareVice || $isHardwareVice)
                                                        <a href="{{ route('profile.show', $member->user_id) }}" class="font-bold text-gray-900 hover:text-indigo-500 transition-colors">{{ $member?->user?->name ?? 'Unknown' }}</a>
                                                    @else
                                                        <p class="font-bold text-gray-900">{{ $member?->user?->name ?? 'Unknown' }}</p>
                                                    @endif
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            ...
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <button onclick="openEvaluationModal('{{ $member?->id }}', '{{ addslashes($member?->user?->name ?? 'Unknown') }}', 'member', '{{ $currentPeriod->id ?? '' }}')"
                                                    class="rounded-xl bg-white border border-gray-200 px-4 py-2 text-xs font-black text-gray-700 hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
                                                    Evaluate
                                                </button>
                                                @endif
                                                ...
                                                <button onclick="openEditTeamModal({{ $member->id }}, {{ $member->team_number ?? 'null' }}, '{{ addslashes($member->user->name ?? 'Unknown') }}')"
                                                    class="rounded-xl bg-gray-100 border border-gray-200 px-3 py-2 text-xs font-black text-gray-700 hover:bg-gray-200 transition-all shadow-sm"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-12 text-center text-gray-500 font-bold italic">
                                            No members found.
                                        </td>
                                    </tr>
                                @endforelse
                        ...
                        {{-- Pagination Footer --}}
                        @if($members->total() > 0)
                        <div class="mt-4 px-6 pb-6 pt-4 border-t border-gray-100 bg-gray-50/30">
            ...
            <div x-show="tab==='subleaders'" x-transition style="display:none;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="relative rounded-2xl border border-gray-200 bg-white p-6 group hover:border-indigo-500 transition-all shadow-sm">
                            ...
                            <div class="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button onclick="openEditTeamModal({{ $sl->id }}, {{ $sl->team_number ?? 'null' }}, '{{ addslashes($sl->user->name ?? 'Unknown') }}')"
                                    class="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-50 text-gray-500 hover:bg-indigo-500 hover:text-white transition-all shadow-sm" title="Edit Team Number">
                            ...
                            <div class="flex items-center gap-4 mb-4">
                                ...
                                @endphp
                                <img src="{{ $slAvatarUrl }}"
                                     class="w-12 h-12 rounded-2xl object-cover bg-white p-1 border border-indigo-100 shadow-sm">
                                <div>
                                    <h4 class="font-black text-gray-900">{{ $sl?->user?->name ?? 'Unknown' }}</h4>
                    ...
                    {{-- Assign Button Card --}}
                    @if($isLeader || $isGeneralVice || $isSoftwareVice || $isHardwareVice)
                    <div class="rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-8 group hover:border-indigo-500 transition-all cursor-pointer bg-gray-50/30"
                        ...
                        <span class="font-black text-xs text-gray-500 uppercase tracking-widest">Assign New Sub Leader</span>
                    </div>
                    @endif
                </div>
            ...
            <div x-show="tab==='viceleaders'" x-transition style="display:none;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse(($viceLeaders ?? collect()) as $vl)
                        <div class="rounded-3xl bg-white border border-gray-200 p-6 shadow-sm group hover:-translate-y-1 transition-all">
                            <div class="flex items-center gap-4 mb-6">
                                ...
                                @endphp
                                <img src="{{ $vlAvatarUrl }}"
                                     class="w-14 h-14 rounded-2xl object-cover bg-white p-1 border border-indigo-100 shadow-md">
                                <div>
                                    <h4 class="font-black text-gray-900 leading-tight">{{ $vl?->user?->name ?? 'Unknown' }}</h4>
                            ...
                            <button onclick="openEvaluationModal('{{ $vl?->id }}', '{{ addslashes($vl?->user?->name ?? 'Unknown') }}', 'vice_leader', '{{ $currentPeriod->id ?? '' }}')"
                                class="w-full rounded-2xl bg-gray-50 border border-gray-100 text-gray-700 font-black py-3 text-xs hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
            ...
            <div x-show="tab==='tasks'" x-transition style="display:none;">
                <div class="rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse(($tasks ?? collect()) as $t)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <p class="font-black text-gray-900 leading-none">{{ $t->title }}</p>
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-1">
                                            @foreach($t->members as $memberName)
                                                <span class="text-[9px] bg-gray-100 px-1.5 py-0.5 rounded-md font-bold text-gray-500">{{ $memberName }}</span>
                                            @endforeach
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-gray-500 font-bold italic">No tasks found for this week.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    @if($tasks->total() > 0)
                    <div class="mt-4 px-6 pb-6 pt-4 border-t border-gray-100 bg-gray-50/20">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">
            ...
            <div x-show="tab==='workshops'" x-transition style="display:none;">
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-lg font-black text-gray-800">Workshops</h2>
                    @if(!$isSubLeader)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @forelse(($workshops ?? collect()) as $w)
                        <div class="rounded-2xl border border-gray-200 bg-white p-6 group hover:border-amber-500 transition-all shadow-sm">
                            <div class="flex items-start justify-between">
                                <div>
                                    <h4 class="font-black text-gray-900 leading-tight text-lg">{{ $w?->title ?? 'Untitled Workshop' }}</h4>
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse(($meetings ?? collect()) as $m)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <p class="font-black text-gray-900 leading-none">{{ $m?->topic ?? 'Untitled Meeting' }}</p>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-[10px] font-black uppercase text-gray-500">{{ $m?->status ?? 'Pending' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        @php
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-center text-gray-500 font-bold italic">No meetings found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
