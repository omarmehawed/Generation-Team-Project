...
<div id="attendanceWorkshopModal" x-data="workshopAttendanceManager()" 
    ...
    <div id="attendanceWorkshopModalContent" 
        class="relative bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl scale-95 opacity-0 transition-all duration-300 transform border border-gray-100 flex flex-col max-h-[90vh]">
        
        <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <div>
                <h3 class="text-xl font-black text-gray-800" id="attendanceWorkshopTitle">Workshop Attendance</h3>
                <p class="text-xs text-gray-500 mt-1 font-medium">Record attendance and participation scores.</p>
            </div>
        ...
        <div class="px-8 py-4 bg-gray-50/30 border-b border-gray-50">
            <div class="relative">
                ...
                <input type="text" x-model="searchQuery" placeholder="Search members by name, email or academic ID..."
                    class="w-full pl-10 pr-4 py-3 rounded-2xl border-2 border-gray-100 bg-white text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder-gray-400">
            </div>
        </div>
        ...
        <form id="attendanceWorkshopForm" @submit.prevent="submitForm()" class="flex flex-col flex-1 overflow-hidden">
            <div class="flex-1 overflow-y-auto p-8 space-y-4 custom-scroll">
                ...
                <div x-show="!isLoading && filteredAttendees.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h4 class="text-gray-500 font-bold">No Members Found</h4>
                ...
                <div x-show="!isLoading && filteredAttendees.length > 0" class="space-y-3">
                    <template x-for="(a, index) in filteredAttendees" :key="a.user_id">
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-3xl border border-gray-100 bg-gray-50/50 hover:border-indigo-200 transition-all group">
                            {{-- Member Info --}}
                            <div class="flex items-center gap-4 sm:w-1/3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-gray-800 truncate" x-text="a.name"></p>
                            ...
                            <div class="flex-1">
                                ...
                                <select x-model="a.status" 
                                    class="w-full bg-white border-2 border-gray-100 rounded-xl px-4 py-2.5 text-xs font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                            ...
                            <div class="w-full sm:w-24" x-show="canScore">
                                ...
                                <input type="number" x-model.number="a.participation_score" min="0" max="10" step="0.5" 
                                    ...
                                    class="w-full bg-white border-2 border-gray-100 rounded-xl px-4 py-2.5 text-xs font-black text-center focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="px-8 py-5 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
                ...
                <div class="flex gap-3">
                    <button type="button" @click="closeModal()" 
                        class="px-5 py-2.5 rounded-xl font-bold text-xs text-gray-500 uppercase tracking-widest transition-colors hover:text-gray-800">Cancel</button>
