...
<div id="assignMemberModal" x-data="assignMemberModalData()"
    ...
    <div id="assignMemberModalContent"
        class="relative bg-white dark:bg-gray-800 rounded-[3rem] w-full max-w-2xl overflow-visible shadow-2xl scale-95 opacity-0 transition-all duration-300 transform border border-gray-100 dark:border-gray-700">

        {{-- Header --}}
        <div
            class="px-8 py-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50">
            <div>
                <h3 class="text-xl font-black text-gray-800 dark:text-gray-200">Add Member to Team</h3>
            ...
            <form id="assignMemberForm" action="{{ route('evaluation.assign-member', $team->id ?? 0) }}" method="POST">
                ...
                <div x-show="step === 1" x-transition.opacity.duration.300ms>
                    <div class="mb-6">
                                ...
                                <div class="max-h-[450px] overflow-y-auto custom-scroll">
                                    <template x-for="m in filteredMembers" :key="m.id">
                                        <div @click="selectMember(m)"
                                             class="px-8 py-6 hover:bg-indigo-600 hover:text-white cursor-pointer flex items-center gap-8 transition-all border-b border-gray-50 last:border-none group">
                                            <img :src="m.avatar" class="w-14 h-14 rounded-2xl border-2 border-white shadow-md group-hover:scale-105 transition-transform">
                        ...
                        <div x-show="selectedMember" x-cloak x-transition
                            class="mt-4 p-5 rounded-2xl border-2 border-indigo-500 bg-indigo-50/50 flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400" x-text="selectedMember?.email"></div>
                                </div>
                            </div>
                    ...
                    <div class="flex justify-end pt-6 border-t border-gray-100 dark:border-gray-700">
                        <button type="button" @click="nextStep()" :disabled="!selectedMember"
                            ...
                            :class="selectedMember ?'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-300 cursor-not-allowed'">
                            Continue
                ...
                <div x-show="step === 2" x-cloak x-transition.opacity.duration.300ms>
                    <div class="mb-6">
                        ...
                        <div class="p-6 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 text-center">
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4 font-medium">
                                @if($isSubLeader)
                                    You are about to add <span class="font-bold text-gray-900 dark:text-gray-100" x-text="selectedMember?.name"></span> to your team module (Team <span class="font-bold text-indigo-500">#{{ $myMember->team_number ?? '-' }}</span>).
                                @else
                                    Select a team number to assign <span class="font-bold text-gray-900 dark:text-gray-100" x-text="selectedMember?.name"></span> to:
                                @endif
                            </p>
                            ...
                            <div class="mt-4 max-w-xs mx-auto">
                                ...
                                <select name="team_number" required class="w-full bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-3 transition-colors">
                            ...
                            <p class="text-[10px] text-gray-500 dark:text-gray-400 italic mt-4">This will allow the corresponding sub-leader to evaluate their weekly performance.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-6 border-t border-gray-100 dark:border-gray-700">
                        <button type="button" @click="step = 1"
                            class="px-5 py-2.5 rounded-xl font-bold text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-900 hover:bg-gray-200 transition-colors">
