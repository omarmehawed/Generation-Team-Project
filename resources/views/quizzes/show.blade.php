...
<div class="max-w-4xl mx-auto py-8">
    <div class="mb-6">
        <a href="{{ route('quizzes.index') }}" class="text-gray-500 hover:text-black font-bold text-sm mb-2 inline-block"><i class="fas fa-arrow-left"></i> All Evaluations</a>
    </div>
        ...
        <div class="p-10">
            @if($attempt && !in_array($attempt->status, ['in_progress', 'pending']) && !$hasApprovedRetry)
                ...
                <div class="{{ $attempt->status == 'disqualified' || $attempt->status == 'cancelled' ? 'bg-red-50 border-red-500' : 'bg-green-50 border-green-500' }} border-2 rounded-2xl p-6 text-center transform transition hover:scale-[1.01]">
                    ...
                    <div class="inline-block bg-white px-6 py-3 rounded-xl shadow-md border {{ $attempt->status == 'disqualified' || $attempt->status == 'cancelled' ? 'border-red-100' : 'border-green-100' }}">
                        <span class="text-gray-500 font-bold uppercase tracking-widest text-xs block mb-1">Your Score</span>
                    ...
                    @if(in_array($attempt->status, ['disqualified', 'cancelled']))\
                        <div class="mt-4 p-4 bg-white/60 rounded-xl text-left border border-red-100 inline-block max-w-lg mx-auto">
                            <p class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-1">Reason for Disqualification:</p>
                ...
                @if(in_array($attempt->status, ['disqualified', 'cancelled', 'submitted']) && $attempt->score == 0)
                    ...
                    @else
                        <div class="mt-8 bg-gray-50 border-2 border-gray-200 rounded-2xl p-6 shadow-sm">
                            <div class="flex items-center gap-4 mb-4">
                                <div class="w-12 h-12 bg-gray-200 text-gray-600 rounded-full flex items-center justify-center shrink-0">
                                    <i class="fas fa-undo-alt text-xl"></i>
                                </div>
                                <div>
                                    <h4 class="text-xl font-black text-gray-800">Request a Retake</h4>
                                    <p class="text-gray-600 text-sm font-bold">If you believe this was a technical error or an unfair disqualification, you can request a retry.</p>
                                </div>
                            </div>
                            ...
                            <form action="{{ route('quizzes.retry', $quiz->id) }}" method="POST">
                                @csrf
                                <textarea name="reason" required rows="3" placeholder="Explain the technical issue or reason you should be granted a retry..." class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 mb-4 text-sm"></textarea>
            ...
            @else
                ...
                <h3 class="text-xl font-black text-gray-800 mb-6 flex items-center gap-2"><i class="fas fa-clipboard-list text-yellow-600"></i> Exam Rules / Instructions</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Duration</p>
                            <p class="font-black text-gray-800 text-lg">{{ $quiz->duration_minutes }} Minutes</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Total Marks</p>
                            <p class="font-black text-gray-800 text-lg">{{ $quiz->total_marks }} Marks</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Fullscreen</p>
                            <p class="font-black text-gray-800 text-lg">{{ $quiz->require_fullscreen ? 'Strict Required' : 'Not Required' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Max Violations</p>
                            <p class="font-black text-gray-800 text-lg">{{ $quiz->max_violations }} Warning(s)</p>
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('quizzes.start', $quiz->id) }}" method="POST">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 pb-2">
                        <label class="flex items-start gap-3 cursor-pointer group flex-1">
                            <input type="checkbox" required class="w-6 h-6 mt-0.5 text-green-600 border-2 border-gray-300 rounded focus:ring-green-500 group-hover:border-green-500 transition">
                            <span class="text-sm font-bold text-gray-700 leading-tight">I have read and understood all instructions. I acknowledge that cheating or violating rules will result in immediate termination of the exam and a zero score.</span>
                        </label>
