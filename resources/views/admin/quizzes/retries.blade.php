...
<div class="max-w-6xl mx-auto py-8">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-gray-900"><i class="fas fa-undo-alt text-yellow-500 mr-2"></i> Quiz Retry Requests</h1>
            <p class="text-gray-500 mt-1">Review requests from members who were disqualified and are asking for another attempt.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100 uppercase tracking-wider text-xs text-gray-500 font-black">
            ...
            <tbody class="divide-y divide-gray-100">
                        ...
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $request->user->department ?? 'Member' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="p-4">
                            <p class="text-sm font-bold text-gray-800">{{ $request->quiz->title }}</p>
                            <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-md">Attempt #{{ $request->attempt->attempt_number ?? 1 }}</span>
                        </td>
                        <td class="p-4">
                            <div class="mb-2">
                            ...
                            <div class="bg-gray-50 p-3 rounded-lg border border-gray-100 text-sm">
                                <span class="font-bold text-gray-500 block mb-1 text-xs uppercase">Member's Reason:</span>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="p-8 text-center text-gray-500 font-bold">No retry requests found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="review-modal" class="fixed inset-0 bg-black/60 z-50 hidden items-center justify-center">
    <div class="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl relative">
        ...
        <h3 class="text-2xl font-black text-gray-900 mb-2">Review Request</h3>
        <p class="text-sm text-gray-500 mb-6 font-bold">Provide optional notes, then approve or reject the member's request to retake the exam.</p>
        
        <label class="block text-sm font-bold text-gray-700 mb-2">Admin Notes (Optional)</label>
        <textarea id="modal-notes" rows="3" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 mb-6" placeholder="Example: Approved due to valid server timeout..."></textarea>
