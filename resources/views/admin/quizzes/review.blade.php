...
@section('content')
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 text-gray-800 dark:text-gray-200">
    <!-- Header Section -->
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div class="flex items-center gap-4 sm:gap-5">
            <a href="{{ route('admin.quizzes.results', $quiz->id) }}" class="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-400 hover:text-indigo-600 hover:shadow-lg transition shadow-sm">
                <i class="fas fa-arrow-left text-sm sm:text-base"></i>
            </a>
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-gray-100 flex items-center gap-2 sm:gap-3">
                ...
                <p class="text-gray-500 dark:text-gray-400 font-bold text-xs sm:text-sm mt-1">Quiz: {{ $quiz->title }} • Attempt #{{ $attempt->attempt_number }}</p>
            </div>
        </div>
        
        <div class="flex flex-wrap items-center gap-3">
            <div class="px-4 py-2 sm:px-6 sm:py-3 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-3">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-10">
        <div class="p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-100/50">
            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Status</p>
            <p class="font-black text-gray-900 dark:text-gray-100 text-sm sm:text-base uppercase">{{ $attempt->status }}</p>
        </div>
        <div class="p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-100/50">
            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Questions</p>
            <p class="font-black text-gray-900 dark:text-gray-100 text-sm sm:text-base">{{ $quiz->questions->count() }} Total</p>
        </div>
        <div class="p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-100/50">
        ...
        <div class="p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-100/50">
            ...
            <p class="font-black text-gray-900 dark:text-gray-100 text-sm sm:text-base">
    ...
    <div class="space-y-8">
        @foreach($questions as $index => $q)
            @php
                ...
                $isUnanswered = !$answer;
            @endphp
            <div class="bg-white dark:bg-gray-800 rounded-[2rem] sm:rounded-[3rem] border-2 {{ $isUnanswered ?'border-gray-100' : ($isCorrect ? 'border-emerald-100 shadow-emerald-50' : 'border-red-100 shadow-red-50') }} shadow-xl overflow-hidden group hover:border-indigo-200 transition duration-300">
                <!-- Question Header -->
                <div class="p-6 sm:p-8 border-b {{ $isUnanswered ?'bg-gray-50/30' : ($isCorrect ? 'bg-emerald-50/30' : 'bg-red-50/30') }} border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3 sm:gap-4">
                        <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-center font-black text-gray-400 text-sm sm:text-lg">
                            {{ $index + 1 }}
                        </div>
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-0.5">
                                ...
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400">{{ $q->marks }} Points</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-6 sm:p-10">
                    <p class="font-bold text-gray-800 dark:text-gray-200 text-lg sm:text-xl leading-relaxed mb-8">{{ $q->question_text }}</p>
                    ...
                    @else
                        ...
                        <div class="mb-6">
                            ...
                            <div class="p-6 rounded-[1.5rem] sm:rounded-[2rem] border-2 border-gray-100 dark:border-gray-700 bg-gray-50/50 shadow-inner">
                                <p class="text-gray-700 dark:text-gray-300 font-bold whitespace-pre-wrap leading-relaxed italic">
                        ...
                        @if($answer && $answer->is_graded)
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between p-6 rounded-2xl bg-indigo-50 border border-indigo-100 gap-4">
                                ...
                                @if($answer->feedback)
                                    <div class="flex-1 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm italic text-xs text-gray-500 dark:text-gray-400">
    ...
    <div class="mt-16 bg-gray-900 rounded-[2.5rem] sm:rounded-[4rem] p-8 sm:p-12 text-center text-white relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col items-center">
            ...
            <a href="{{ route('admin.quizzes.results', $quiz->id) }}" class="mt-10 px-10 py-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest hover:bg-gray-100 transition shadow-xl shadow-white/5 active:scale-95 inline-block">
