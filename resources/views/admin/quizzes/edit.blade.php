...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 p-6 sm:p-10 md:p-12 border border-gray-100 dark:border-gray-700">
        <div class="flex items-center gap-4 mb-10 pb-6 border-b border-gray-50">
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-gray-100">Edit: {{ $quiz->title }}</h1>
        </div>
        ...
        <form action="{{ route('admin.quizzes.update', $quiz->id) }}" method="POST" class="space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ $quiz->title }}" required class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 font-bold text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Description</label>
                    <textarea name="description" rows="4" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 font-bold text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">{{ $quiz->description }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="duration_minutes" required min="1" value="{{ $quiz->duration_minutes }}" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 pl-12 font-black text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="total_marks" required min="0" value="{{ $quiz->total_marks }}" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 pl-12 font-black text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Available From</label>
                    <input type="datetime-local" name="start_at" value="{{ $quiz->start_at ? $quiz->start_at->format('Y-m-d\TH:i') : '' }}" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 font-bold text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Available Until</label>
                    <input type="datetime-local" name="end_at" value="{{ $quiz->end_at ? $quiz->end_at->format('Y-m-d\TH:i') : '' }}" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 font-bold text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                </div>
            </div>
            ...
            <div class="pt-8 border-t border-gray-50">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50/50 p-6 sm:p-8 rounded-[2rem] border border-gray-100 dark:border-gray-700 max-h-80 overflow-y-auto">
                    <label class="flex items-center gap-4 cursor-pointer p-4 bg-white dark:bg-gray-800 rounded-2xl border-2 border-transparent hover:border-indigo-100 transition shadow-sm group">
                        <input type="checkbox" name="targeted_roles[]" value="all" {{ in_array('all', $rolesArr) ? 'checked' : '' }} class="w-6 h-6 text-indigo-600 rounded-lg border-gray-200 dark:border-gray-700 focus:ring-indigo-500 targeted-role-all">
                        <span class="font-black text-gray-800 dark:text-gray-200 text-sm">All Team Members</span>
                    </label>
                    @foreach($rolesList as $val => $label)
                        <label class="flex items-center gap-4 cursor-pointer p-4 bg-white dark:bg-gray-800 rounded-2xl border-2 border-transparent hover:border-indigo-100 transition shadow-sm group">
                            <input type="checkbox" name="targeted_roles[]" value="{{ $val }}" {{ in_array($val, $rolesArr) ? 'checked' : '' }} class="w-6 h-6 text-indigo-600 rounded-lg border-gray-200 dark:border-gray-700 focus:ring-indigo-500 targeted-role-spec">
                    ...
                    <div class="md:col-span-1">
                        <label class="flex flex-col h-full bg-white dark:bg-gray-800 rounded-[2rem] border-2 border-gray-100 dark:border-gray-700 p-6 hover:border-orange-200 transition cursor-pointer group shadow-sm">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <input type="checkbox" name="require_fullscreen" value="1" {{ $quiz->require_fullscreen ? 'checked' : '' }} class="w-6 h-6 text-orange-500 rounded-lg border-gray-200 dark:border-gray-700 focus:ring-orange-400">
                            </div>
                    ...
                    <div class="md:col-span-1">
                        <label class="flex flex-col h-full bg-white dark:bg-gray-800 rounded-[2rem] border-2 border-gray-100 dark:border-gray-700 p-6 hover:border-purple-200 transition cursor-pointer group shadow-sm">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <input type="checkbox" name="shuffle_questions" value="1" {{ $quiz->shuffle_questions ? 'checked' : '' }} class="w-6 h-6 text-purple-600 rounded-lg border-gray-200 dark:border-gray-700 focus:ring-purple-400">
                            </div>
                    ...
                    <div class="md:col-span-1">
                        <label class="flex flex-col h-full bg-white dark:bg-gray-800 rounded-[2rem] border-2 border-gray-100 dark:border-gray-700 p-6 hover:border-purple-200 transition cursor-pointer group shadow-sm">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <input type="checkbox" name="shuffle_options" value="1" {{ $quiz->shuffle_options ? 'checked' : '' }} class="w-6 h-6 text-purple-600 rounded-lg border-gray-200 dark:border-gray-700 focus:ring-purple-400">
                            </div>
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                            <div class="space-y-4">
                                <label class="flex items-center gap-4 cursor-pointer group">
                                    <input type="checkbox" name="auto_cancel_on_copy" value="1" {{ $quiz->auto_cancel_on_copy ? 'checked' : '' }} class="w-6 h-6 text-red-600 rounded-lg border-gray-300 dark:border-gray-600">
                                    <span class="font-black text-gray-800 dark:text-gray-200 text-sm block">Auto-DQ on Copy</span>
                                </label>
                                <label class="flex items-center gap-4 cursor-pointer group">
                                    <input type="checkbox" name="auto_cancel_on_paste" value="1" {{ $quiz->auto_cancel_on_paste ? 'checked' : '' }} class="w-6 h-6 text-red-600 rounded-lg border-gray-300 dark:border-gray-600">
                            ...
                            <div class="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-red-50 flex-1 lg:max-w-xs">
                                ...
                                <input type="number" name="max_violations" min="1" value="{{ $quiz->max_violations }}" class="w-full rounded-xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-3 font-black text-red-600 focus:border-red-500 focus:ring-red-100 transition">
                            </div>
                        </div>
                    </div>
                </div>
