...
@section('content')
<div class="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8 text-gray-800 dark:text-gray-200" x-data="{
    tab: 'list',
    questionType: 'mcq',
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-gray-100">Manage Questions</h1>
        </div>
    ...
    <div x-show="tab === 'list'" class="space-y-6">
        @forelse($quiz->questions as $i => $question)
            <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-xl shadow-gray-100/50 border border-gray-100 dark:border-gray-700 p-6 sm:p-8 flex flex-col md:flex-row justify-between gap-8 group hover:border-indigo-100 transition duration-300">
                <div class="flex-1">
                    <div class="flex flex-wrap items-center gap-3 mb-4">
                        ...
                        <span class="bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg">{{ $question->question_type }}</span>
                    ...
                    <p class="font-black text-lg sm:text-xl text-gray-800 dark:text-gray-200 mb-6 leading-tight">{{ $question->question_text }}</p>
                    ...
                    @else
                        <div class="p-6 bg-gray-50/50 border border-gray-100 dark:border-gray-700 rounded-2xl flex items-center gap-3 text-gray-400 font-bold text-sm italic">
                ...
                <div class="flex md:flex-col items-center justify-end gap-3 pt-6 md:pt-0 border-t md:border-t-0 md:border-l border-gray-50 md:pl-8">
                    <a href="{{ route('admin.quizzes.questions.edit', [$quiz->id, $question->id]) }}" class="flex-1 md:flex-none w-12 h-12 flex items-center justify-center bg-gray-50 dark:bg-gray-900 text-indigo-400 rounded-2xl hover:bg-indigo-50 hover:text-indigo-600 transition shadow-sm" title="Edit Question">
        ...
        @empty
            <div class="text-center py-24 bg-white dark:bg-gray-800 rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-gray-700">
                <div class="w-20 h-20 rounded-[2rem] bg-gray-50 dark:bg-gray-900 flex items-center justify-center text-4xl text-gray-200 mx-auto mb-6">
    ...
    <!-- Add Question Form -->
    <div x-show="tab === 'add'" x-cloak class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 p-8 sm:p-12 border border-gray-100 dark:border-gray-700">
        <div class="flex items-center gap-4 mb-10 pb-6 border-b border-gray-50">
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-gray-100 leading-tight">Create New Question</h2>
        </div>
        ...
        <form action="{{ route('admin.quizzes.questions.store', $quiz->id) }}" method="POST" class="space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <textarea name="question_text" required rows="4" placeholder="What is the output of the following code snippet?" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-6 font-bold text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition"></textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="question_type" x-model="questionType" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 font-black text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="marks" required min="0" step="0.5" value="1" class="w-full rounded-2xl border-gray-100 dark:border-gray-700 bg-gray-50/50 p-4 pl-12 font-black text-gray-800 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-200 transition">
                    </div>
                </div>
            </div>

            <!-- Options Section (For MCQ) -->
            <div x-show="questionType === 'mcq'" class="bg-gray-50/50 p-8 rounded-[2rem] border border-gray-100 dark:border-gray-700">
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center gap-3">
                        <div class="w-1.5 h-6 bg-indigo-600 rounded-full"></div>
                        <h3 class="text-xs font-black text-gray-900 dark:text-gray-100 uppercase tracking-[0.2em]">Answer Config</h3>
                    </div>
                ...
                <div class="space-y-4">
                    <template x-for="(opt, index) in options" :key="index">
                        <div class="flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm group hover:border-indigo-200 transition">
                            <div class="flex items-center gap-4 w-full sm:w-auto">
                                <label class="flex items-center cursor-pointer group/radio">
                                    <input type="radio" required name="correct_option" :value="index" :checked="opt.isCorrect" class="w-6 h-6 text-emerald-500 focus:ring-emerald-200 border-gray-200 dark:border-gray-700">
                            ...
                            <input type="text" :name="'options['+index+'][text]'" x-model="opt.text" required class="flex-1 w-full bg-gray-50/50 border-transparent rounded-xl focus:border-indigo-100 focus:ring-0 font-bold text-gray-700 dark:text-gray-300" placeholder="Type answer choice...">
            ...
            <div class="flex items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <input type="checkbox" name="is_required" value="1" checked id="isReq" class="w-6 h-6 text-indigo-600 rounded-lg border-gray-200 dark:border-gray-700">
                <label for="isReq" class="flex-1 pt-1">
                    <span class="block font-black text-gray-800 dark:text-gray-200 text-sm">Strict Requirement</span>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-8 border-t border-gray-50">
                <button type="button" @click="tab = 'list'" class="w-full sm:w-auto px-8 py-4 bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-100 transition">Cancel</button>
