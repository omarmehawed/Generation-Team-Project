...
@section('content')
<div class="max-w-7xl mx-auto py-8 text-gray-800" x-data="liveObserver('{{ $attempt->id }}')" x-init="startSync()">
    <!-- Header -->
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div class="flex items-center gap-5">
            <a href="{{ route('admin.quizzes.live', $attempt->quiz_id) }}" class="w-12 h-12 flex items-center justify-center rounded-2xl bg-white border border-gray-100 text-gray-400 hover:text-indigo-600 hover:shadow-lg transition shadow-sm">
            ...
            <div>
                ...
                <p class="text-gray-500 font-bold text-sm mt-1">Quiz: {{ $attempt->quiz->title }} • Attempt #{{ $attempt->attempt_number }}</p>
            </div>
        </div>
        
        <div class="flex items-center gap-3 px-6 py-4 bg-white rounded-[1.5rem] shadow-sm border border-gray-100">
            <template x-if="isSyncing">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-black text-gray-600 uppercase tracking-widest">Live Syncing</span>
                </div>
            </template>
            <template x-if="!isSyncing">
                <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        ...
        <!-- Violations -->
        <div class="p-6 rounded-[2.5rem] bg-white border border-gray-100 shadow-xl shadow-gray-100/50 transform hover:scale-[1.02] transition duration-300">
            <div class="flex items-center justify-between mb-4">
            ...
            <p class="text-3xl font-black text-gray-900 leading-none mb-1" x-text="detail.violation_count">0</p>
        ...
        <div class="p-6 rounded-[2.5rem] bg-white border border-gray-100 shadow-xl shadow-gray-100/50 transform hover:scale-[1.02] transition duration-300">
            ...
            <p class="text-3xl font-black text-gray-900 leading-none mb-1" x-text="detail.current_step + '/' + detail.last_page"></p>
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-tighter">Question Groups</p>
        </div>
        ...
        <div class="p-6 rounded-[2.5rem] bg-white border border-gray-100 shadow-xl shadow-gray-100/50 transform hover:scale-[1.02] transition duration-300">
            ...
            <p class="text-xl font-black text-gray-900 leading-none mb-2 mt-1 truncate" x-text="detail.last_activity"></p>
            <p class="text-[10px] font-bold text-emerald-500 uppercase tracking-tighter">Last Response Sync</p>
        </div>
    </div>

    <!-- Main Content -->
    <div class="bg-white rounded-[3rem] border border-gray-100 shadow-2xl p-8 md:p-12">
        <div class="flex items-center gap-4 mb-10 pb-6 border-b border-gray-50">
            <div class="w-2 h-8 bg-indigo-600 rounded-full"></div>
            <h2 class="text-xl font-black text-gray-900 uppercase tracking-widest">Active Question Group</h2>
        </div>
        ...
        <div x-show="loading" class="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p class="text-gray-900 font-black uppercase tracking-[0.3em] text-xs">Capturing Live Stream...</p>
        </div>

        <div x-show="!loading" class="space-y-12">
            <template x-for="(q, idx) in questions" :key="q.id">
                <div class="bg-gray-50/50 rounded-[3rem] border border-gray-100 p-8 md:p-10 hover:bg-white hover:shadow-2xl transition duration-500 group">
                    <div class="flex flex-wrap justify-between items-start gap-4 mb-8">
                        <div class="inline-flex items-center gap-3 px-5 py-2.5 bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="mb-10">
                        <p class="font-black text-gray-800 text-2xl leading-relaxed" x-text="q.question_text"></p>
                    </div>
                            ...
                            <template x-for="opt in q.options" :key="opt.id">
                                <div class="p-6 rounded-3xl border-2 font-bold flex items-center gap-4 transition-all duration-300"
                                    ...
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center border-2" 
                                        ...
                                        <div class="w-2.5 h-2.5 rounded-full bg-white" x-show="answers[q.id]?.selected_option_id == opt.id"></div>
                                    </div>
                    ...
                    <template x-if="q.question_type === 'written'">
                        <div class="p-8 rounded-[2rem] border-2 border-gray-100 bg-white group-hover:border-indigo-100 transition shadow-inner">
                            ...
                            <p class="text-bases font-bold text-gray-700 whitespace-pre-wrap leading-relaxed min-h-[80px]" x-text="answers[q.id]?.text_answer || 'No input recorded yet. Awaiting candidate interaction...'"></p>
                        </div>
                    </template>
                </div>
            </template>
