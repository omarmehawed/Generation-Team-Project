...
@section('content')
<div class="max-w-4xl mx-auto py-8 text-gray-800" x-data="{
    ...
    <div class="mb-6">
        <a href="{{ route('admin.quizzes.questions', $quiz->id) }}" class="text-gray-500 hover:text-black font-bold text-sm mb-2 inline-block"><i class="fas fa-arrow-left"></i> Back to Questions</a>
    </div>

    <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        <h2 class="text-xl font-bold text-gray-800 mb-6 border-b pb-4"><i class="fas fa-edit text-blue-500"></i> Edit Question</h2>
        ...
        <form action="{{ route('admin.quizzes.questions.update', [$quiz->id, $question->id]) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="col-span-2">
                    ...
                    <textarea name="question_text" required rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-200">{{ $question->question_text }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="marks" required min="0" step="0.5" value="{{ $question->marks }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-200">
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 mb-6">
                <input type="checkbox" name="is_required" value="1" {{ $question->is_required ? 'checked' : '' }} id="isReq" class="text-yellow-600 rounded border-gray-300">
                <label for="isReq" class="font-bold text-sm text-gray-700">Question is Required to answer</label>
            </div>

            <div class="text-right border-t pt-4">
                <a href="{{ route('admin.quizzes.questions', $quiz->id) }}" class="inline-block bg-gray-100 text-gray-600 font-bold px-4 py-2 rounded-xl border hover:bg-gray-200 mr-2">Cancel</a>
