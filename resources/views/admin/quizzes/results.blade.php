...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-gray-100 flex items-center gap-4">
        ...
        <div class="flex flex-wrap gap-3 w-full sm:w-auto">
            <button onclick="openParticipationModal()" class="flex-1 sm:flex-none justify-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-indigo-100 hover:text-indigo-600 text-gray-600 dark:text-gray-400 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-sm transition flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-xl shadow-gray-100/50 overflow-hidden border border-gray-100 dark:border-gray-700">
                    ...
                    <tr class="hover:bg-gray-50/50 transition duration-200">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-900 flex items-center justify-center text-gray-400 font-black">
                                    {{ strtoupper(substr($attempt->user->name, 0, 1)) }}
                                </div>
                                <div>
                                    <p class="font-black text-gray-900 dark:text-gray-100">{{ $attempt->user->name }}</p>
                        ...
                        <td class="px-6 py-6">
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400">{{ $attempt->started_at ? $attempt->started_at->format('M d, h:i A') : 'N/A' }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <i class="far fa-check-circle text-gray-300"></i>
                                    <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400">{{ $attempt->submitted_at ? $attempt->submitted_at->format('M d, h:i A') : 'N/A' }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.quizzes.attempts.review', $attempt->id) }}" class="w-10 h-10 flex items-center justify-center bg-gray-50 dark:bg-gray-900 text-indigo-500 rounded-xl hover:bg-indigo-600 hover:text-white transition shadow-sm" title="Review Answers">
...
<div id="participation-modal" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white dark:bg-gray-800 w-full max-w-5xl max-h-[90vh] rounded-[2.5rem] overflow-hidden shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col border border-white">
        {{-- Header --}}
        <div class="p-8 sm:p-10 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/30">
            <div>
                <h3 class="text-2xl font-black text-gray-900 dark:text-gray-100 flex items-center gap-4">
            ...
            <button onclick="closeParticipationModal()" class="w-12 h-12 flex items-center justify-center bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl text-gray-400 hover:text-rose-500 hover:border-rose-100 transition shadow-sm"><i class="fas fa-times"></i></button>
        </div>

        {{-- Filters --}}
        <div class="px-8 py-6 bg-white dark:bg-gray-800 border-b border-gray-50">
            <div class="flex flex-wrap gap-2">
                ...
                <button onclick="showParticipationTab('not_started')" class="participation-tab-btn px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition text-gray-400 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 flex items-center gap-2" data-tab="not_started">
        ...
        <div class="flex-1 overflow-y-auto px-8 py-10 bg-gray-50/20">
            <div id="participation-items-container" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="participation-item p-6 bg-white dark:bg-gray-800 rounded-3xl border border-emerald-100 shadow-sm flex items-start justify-between group hover:border-emerald-300 transition duration-300" data-status="completed">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="overflow-hidden">
                            <p class="font-black text-gray-800 dark:text-gray-200 text-sm truncate w-full">{{ $item['user']->name }}</p>
                ...
                <div class="participation-item p-6 bg-white dark:bg-gray-800 rounded-3xl border border-indigo-100 shadow-sm flex items-start justify-between group hover:border-indigo-300 transition duration-300" data-status="in_progress">
                    <div class="flex items-start gap-4">
                ...
                <div class="participation-item p-6 bg-white/60 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-start justify-between group hover:bg-white hover:border-gray-300 transition duration-300" data-status="not_started">
                    <div class="flex items-start gap-4 opacity-50">
                        <div class="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-900 text-gray-400 flex items-center justify-center font-black text-xl shadow-inner">
                            {{ strtoupper(substr($item['user']->name, 0, 1)) }}
                        </div>
                        <div class="overflow-hidden">
                            <p class="font-black text-gray-800 dark:text-gray-200 text-sm truncate w-full">{{ $item['user']->name }}</p>
...
<div id="extra-time-modal" class="fixed inset-0 bg-black/60 z-50 hidden items-center justify-center">
    <div class="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl p-6 shadow-2xl">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-black text-gray-900 dark:text-gray-100"><i class="fas fa-clock text-yellow-500 mr-2"></i> Grant Extra Time</h3>
            <button onclick="closeExtraTimeModal()" class="text-gray-400 hover:text-black text-xl"><i class="fas fa-times"></i></button>
        </div>

        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Member: <strong id="modal-member-name" class="text-gray-900 dark:text-gray-100"></strong></p>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Unanswered questions: <strong id="modal-unanswered-count" class="text-amber-600"></strong></p>

        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Extra time duration</label>
        <div class="flex gap-2 mb-4">
            <button type="button" onclick="setMinutes(5)" class="flex-1 bg-gray-100 dark:bg-gray-900 hover:bg-yellow-100 text-gray-700 dark:text-gray-300 font-bold py-2 rounded-lg text-sm transition">5 min</button>
            <button type="button" onclick="setMinutes(10)" class="flex-1 bg-gray-100 dark:bg-gray-900 hover:bg-yellow-100 text-gray-700 dark:text-gray-300 font-bold py-2 rounded-lg text-sm transition">10 min</button>
            <button type="button" onclick="setMinutes(15)" class="flex-1 bg-gray-100 dark:bg-gray-900 hover:bg-yellow-100 text-gray-700 dark:text-gray-300 font-bold py-2 rounded-lg text-sm transition">15 min</button>
            <button type="button" onclick="setMinutes(30)" class="flex-1 bg-gray-100 dark:bg-gray-900 hover:bg-yellow-100 text-gray-700 dark:text-gray-300 font-bold py-2 rounded-lg text-sm transition">30 min</button>
        </div>

        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Custom (minutes)</label>
        <input type="number" id="modal-minutes-input" min="1" max="120" value="10" class="w-full rounded-xl border-gray-300 dark:border-gray-600 focus:border-yellow-500 focus:ring-yellow-400 mb-4 text-sm">

        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Admin Note (Optional)</label>
        <textarea id="modal-notes-input" rows="2" class="w-full rounded-xl border-gray-300 dark:border-gray-600 focus:border-yellow-500 focus:ring-yellow-400 mb-5 text-sm" placeholder="e.g. Approved due to network issue..."></textarea>

        <div class="flex gap-3">
            <button onclick="closeExtraTimeModal()" class="flex-1 bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-200 font-bold py-3 rounded-xl transition">Cancel</button>
