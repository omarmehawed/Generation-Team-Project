...
<div class="max-w-3xl mx-auto px-4 py-8">
    <div class="mb-6">
        <a href="{{ route('forms.member.index') }}" class="text-gray-500 hover:text-gray-700 flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 border-t-8 border-t-[#2596be] mb-6">
    ...
    <form action="{{ route('forms.member.store', $form->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        @foreach($form->questions as $question)
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 {{ $errors->has('answers.'.$question->id) ? 'border-red-500' : '' }}">
                ...
                @php 
                    $ans = old('answers.'.$question->id);
                    if ($existingResponse) {
                        $savedAns = $existingResponse->answers->where('form_question_id', $question->id)->first();
                        if ($savedAns && !$ans) {
                            $ans = $question->type === 'checkboxes' ? $savedAns->answer_json : $savedAns->answer_text;
                        }
                    }
                @endphp
                ...
                @elseif($question->type === 'file_upload')
                    <div x-data="{ previewUrl: '' }" class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 text-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                        <input type="file" name="answers[{{ $question->id }}]" id="file_{{ $question->id }}" class="hidden" 
                               ...
                               {{ $question->is_required && !($existingResponse && $existingResponse->answers->where('form_question_id', $question->id)->first()?->answer_file) ? 'required' : '' }}>
                        <label for="file_{{ $question->id }}" class="cursor-pointer block">
                            ...
                            @if($existingResponse)
                                @php 
                                    $savedFile = $existingResponse->answers->where('form_question_id', $question->id)->first()?->answer_file; 
                                    $isImage = $savedFile && preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $savedFile);
                                @endphp
                                @if($savedFile)
                                    <div class="mt-3 text-sm text-green-600 font-medium" x-show="!previewUrl">
                                        <i class="fas fa-check-circle"></i> File uploaded. (Upload new to replace)
                                        @if($isImage)
                                            <div class="mt-4 flex justify-center">
                                                <img src="{{ Storage::url($savedFile) }}" class="max-h-48 rounded-lg shadow-sm object-contain" alt="Uploaded Image">
                                            </div>
                                        @endif
                                    </div>
                                @endif
        ...
        <div class="flex justify-end pt-4 mb-16">
            <button type="submit" class="btn-primary bg-[#2596be] hover:bg-[#1a7a9c] text-white px-8 py-3 rounded-xl font-bold transition flex items-center gap-2 shadow-lg shadow-[#2596be]/30 text-lg">
